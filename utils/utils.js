 ...
 * @returns {number}
 */
export function calculateJaroWinklerDistance(s1, s2) {
 ...
 * @returns {boolean}
 */
export function isGoodMatch(originalTitle, replacementTitle, similarityThreshold = 0.5) {
  if (!replacementTitle) {
    return false;
  }

  try {
    const score = calculateJaroWinklerDistance(originalTitle, replacementTitle);
